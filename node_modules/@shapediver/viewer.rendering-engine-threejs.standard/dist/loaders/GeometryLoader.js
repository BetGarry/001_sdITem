...
class GeometryLoader {
    ...
    load(geometry, parent, newChild, skeleton) {
        ...
        const c = parent.children.find(c => { c.SDid === obj.SDid && c.SDversion === obj.SDversion; });
        if (c)
            parent.remove(c);
    ...
    createMesh(obj, geometry, threeGeometry, material, materialSettings, skeleton) {
        if (geometry.primitive.mode === viewer_shared_types_1.PRIMITIVE_MODE.POINTS) {
            ...
            throw new viewer_shared_services_1.ShapeDiverViewerDataProcessingError(`GeometryLoader.load: Unrecognized primitive mode ${geometry.primitive.mode}.`);
        }
        obj.traverse(m => {
        ...
        if (bufferAttribute.sparse) {
            ...
            for (let i = 0, il = bufferAttribute.sparseIndices.length; i < il; i++) {
                ...
                if (bufferAttribute.itemSize >= 5)
                    throw new viewer_shared_services_1.ShapeDiverViewerDataProcessingError(`GeometryLoader.loadGeometry: Unsupported itemSize in sparse BufferAttribute.`);
            }
        }
        return buffer;
    }
